...
read_file_after_q3()
# Відповідь на третє питання і запис четвертого питання – Колдомасов Максим

def add_answer3_and_question4():
    try:
        ...
        with open("team_work.txt", "a", encoding="utf-8") as file:
            file.write("\nВідповідь №3: Колдомасов Максим\n")
            file.write(
                "Цикл for у Python — це конструкція, яка дозволяє проходити по елементах "
                "будь-якої послідовності (списку, рядка, словника, множини або діапазону). "
                "На кожній ітерації цикл бере наступний елемент і виконує тіло циклу. "
                "Приклад:\n"
                "for x in [1, 2, 3]:\n"
                "    print(x)  # виведе 1, 2, 3\n"
            )

            file.write("\nПитання №4: Колдомасов Максим\n")
            file.write("4. Що таке функція у Python і як її оголосити?\n")

        print("Відповідь №3 і питання №4 успішно додано!")
...
def read_file_after_q4():
    try:
        with open("team_work.txt", "r", encoding="utf-8") as file:
            print("\nВміст файлу після додавання відповіді №3 і питання №4:")
            print("----------------------------")
            print(file.read())
    except Exception as e:
        print("Помилка:", e)
